...
For maximum utility and efficient management of topics, it is recommended that ``data`` are published to a detailed level of the topic hierarchy. This helps avoid the "pollution" of data messages under the primary topics.  Publishing to a detailed level is realized by selecting a "leaf" topic, which is a topic with no additional subtopics.
