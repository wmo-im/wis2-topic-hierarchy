...
The WIS2 Topic Hierarchy is included in the discovery metadata record <<see section X>>, made available by WIS brokers <<see Y>>, and selected by users to subscribe and receive notifications <<see Z>>. 
...
//change .."and services" to "..and resources"?
The primary topics apply to all data and services in WIS.  They are relational, meaning that any combination of the values in each level can be used to construct a topic applicable to a notification.
...
The representation is encoded as a simple text string of values in each topic level separated by a "/". For example, ``origin/a/wis2/data/ca-eccc/core/weather/surface-based-observations/synop`` or ``origin/a/wis2/data/ca-eccc/recommended/atmospheric-composition/experimental/space-based-observation/geostationary/solar-flares``.
...
.WTH example table of sub-discipline topics
|===
|Level 8 topics|Level 9 topics|Level 10 topics
...
== Resources
//I don't think we need more examples for the WTH

=== All topics
A zip file of all the approved topics is available at https://schemas.wmo.int/wis2/wth/a/1.0/wth.zip.

A browseable and searchable codelist of all topics can be found at https://codes.wmo.int/wis2/wth.

=== Compliance
Conformance with this standard shall be checked using the tests specified the Conformance Class Abstract Test Suite (Normative). It is published at https://schemas.wmo.int/wis2/wth/ats.
